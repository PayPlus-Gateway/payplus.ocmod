...
class ControllerExtensionPaymentPayPlus extends Controller {
	...
	public function index() {
		...
		$data['action'] = $this->url->link('extension/payment/payplus', 'user_token=' . $this->session->data['user_token'], true);
	...
	public function install() {
		if ($this->user->hasPermission('modify','extension/payment/payplus')) {
			$this->load->model('extension/payment/payplus');
			$this->model_extension_payment_payplus->install();
		}
	}
	...
	public function order() {
		...
		$data['payplus_order_info'] = $this->model_extension_payment_payplus->getOrder($order_id);
		
		$maxToCharge=$this->model_extension_payment_payplus->maxToCharge($order_id);
		$data['max_to_charge'] = ($maxToCharge > $data['original_order_info']['total'] ? (double)$data['original_order_info']['total'] : (double)$maxToCharge);
		$data['max_to_refund'] = (double)$this->model_extension_payment_payplus->maxToRefund($order_id);
		...
		$data['refund_url'] = $this->url->link('extension/payment/payplus/refund', 'user_token=' . $this->session->data['user_token'].'&order_id='.$order_id, true);
	...
	public function charge() {
		$this->load->model('extension/payment/payplus');
		$order_info = $this->model_extension_payment_payplus->getOrder($this->request->get['order_id']);
		
		if ($this->config->get('payment_payplus_sandbox')) $apiURL = 'https://restapidev.payplus.co.il/api/v1.0/Transactions/ChargeByTransactionUID';
			else $apiURL = 'https://restapi.payplus.co.il/api/v1.0/Transactions/ChargeByTransactionUID';
		...
		if ($req->data->transaction->status_code == "000" && $req->results->status == "success") {
			...
			$transaction_id=$this->model_extension_payment_payplus->addTransaction($order_info['customer_id'],$transaction_data);
	...
	public function refund() {
		...
		if ($this->config->get('payment_payplus_sandbox')) $apiURL = 'https://restapidev.payplus.co.il/api/v1.0/Transactions/RefundByTransactionUID';
			else $apiURL = 'https://restapi.payplus.co.il/api/v1.0/Transactions/RefundByTransactionUID';

		$tranz_by_payplus_uid=$this->model_extension_payment_payplus->getByPayPlusTransaction($this->request->post['transaction_uid']);
		
		$this->model_extension_payment_payplus->updatePayPlusTransaction($this->request->post['transaction_uid']);
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'extension/payment/payplus')) $this->error['warning'] = $this->language->get('error_permission');
