...
class ControllerExtensionPaymentPayPlus extends Controller {
	...
	public function index() {
		...
		if ($this->customer->isLogged()) { // If User Is Logged In
            ...
            $this->load->model('extension/credit_card/payplus');
			
            $data['cards'] = $this->model_extension_credit_card_payplus->getCards($this->customer->getId());
			}
		$data['action'] = $this->url->link('extension/payment/payplus/create_link', '', true);
    ...
    public  function getTotalAll(){
 	...
 	public function create_link($token=false) {
		...
		if ($data['sandbox_mode']) $apiURL = 'https://restapidev.payplus.co.il/api/v1.0/PaymentPages/generateLink';
			else $apiURL = 'https://restapi.payplus.co.il/api/v1.0/PaymentPages/generateLink';
		
		$refURL_success = $this->url->link('extension/payment/payplus/callback');
	...
	public function callback(){
		...
		if ($request['token_uid'] && $this->customer->isLogged() && $this->session->data['save_card'] && $request['method'] && $request['status'] == "approved" && $request['status_code'] == "000" && $request['token_uid']) { // Save Token if choosed
			$this->load->model('extension/credit_card/payplus');
			$card_data = Array();
		...
		if (!$this->model_extension_payment_payplus->checkOrder($order_id)) {
			$order_data = Array();
			if ($request['used_token']) $saved_card=$request['used_token'];
			...
			$save_order = $this->model_extension_payment_payplus->addOrder($this->customer->getId(),$order_data);
			if ($this->config->get('payment_payplus_logger')) $this->log->write('PAYPLUS :: Card Saved: ' . print_r($save_order,true));
			}
		if (!$this->model_extension_payment_payplus->checkTransaction($this->customer->getId(),$order_id)) {
			...
			$transaction_id=$this->model_extension_payment_payplus->addTransaction($this->customer->getId(),$transaction_data);
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			if ($IPNResponse) {
				...
				$this->model_checkout_order->addOrderHistory($order_id, $order_failure_status_id);
